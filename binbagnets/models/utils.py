...
__all__ = ['plot_heatmap', 'generate_heatmap_pytorch']
...
def plot_heatmap(heatmap, original, ax, cmap='RdBu_r', 
    ...
    if original is not None:
        ...
        original_greyscale = original if len(original.shape) == 2 else \
                                         np.mean(original, axis=-1)
        in_image_upscaled = transform.rescale(original_greyscale, dilation,
                                              mode='constant', 
                                              multichannel=False,
                                              anti_aliasing=True)
    ...
    ax.imshow(heatmap, extent=extent, interpolation='none', cmap=cmap,
              vmin=-abs_min, vmax=abs_max)
    if overlay is not None:
        ax.imshow(overlay, extent=extent, interpolation='none',
                  cmap=cmap_original, alpha=alpha)
...
def generate_heatmap_pytorch(model, image, target, patchsize):
    ...
    with torch.no_grad():
        ...
        padded_image[:, (patchsize-1)//2:(patchsize-1)//2 + x,
                     (patchsize-1)//2:(patchsize-1)//2 + y] = image[0]
